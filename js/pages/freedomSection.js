...
// into one running ticker -- same mechanism as vaultSection.js.
async function communityTicker() {
  const highlights = await freedomStore.highlights.getActive();
  const approved = await freedomCommentsStore.getApprovedAll();
  const items = [
...
function wireHighlightsManage(container, rerender) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!text) return;
    await freedomStore.highlights.create({ text });
    await rerender();
  });
  ...
  container.querySelectorAll('[data-hl-action]').forEach((btn) => {
    btn.addEventListener('click', async () => {
      ...
      if (action === 'move-up') await freedomStore.highlights.moveUp(id);
      else if (action === 'move-down') await freedomStore.highlights.moveDown(id);
      else if (action === 'delete') {
        if (!window.confirm('למחוק את המשפט?')) return;
        await freedomStore.highlights.remove(id);
      } else if (action === 'toggle-active') {
        const all = await freedomStore.highlights.getAll();
        const current = all.find((h) => h.id === id);
        if (current) await freedomStore.highlights.update(id, { isActive: !current.isActive });
      }
      await rerender();
    });
  });
...
async function saveLessonField(id, field, value) {
  await freedomStore.lessons.update(id, { [field]: value });
}
...
export async function mountFreedomHome(container, section, session) {
  async function render() {
    ...
    const lessons = editMode ? await freedomStore.lessons.getAll() : await freedomStore.lessons.getActive();
    ...
    const highlights = editMode ? await freedomStore.highlights.getAll() : [];
    ...
    wireReorderButtons(container, {
      onMove: (id, dir) => (dir === 'up' ? freedomStore.lessons.moveUp(id) : freedomStore.lessons.moveDown(id)),
      rerender: render,
    });
    ...
    if (addForm) {
      addForm.addEventListener('submit', async (e) => {
        ...
        if (!title) return;
        await freedomStore.lessons.create({ title });
        await render();
      });
    }
...
export async function mountFreedomLesson(container, section, lesson, session) {
  async function render() {
    ...
    const approvedComments = await freedomCommentsStore.getApprovedByLessonId(lesson.id);
    const freshLesson = (await freedomStore.lessons.getById(lesson.id)) || lesson;
    const editMode = auth.isEditMode();
    ...
    container.querySelectorAll('[data-desc-mode]').forEach((btn) => {
      btn.addEventListener('click', async () => {
        await freedomStore.lessons.update(freshLesson.id, { descriptionMode: btn.dataset.descMode });
        await render();
      });
    });
    ...
    form.addEventListener('submit', async (e) => {
      ...
      await freedomCommentsStore.create({
        lessonId: lesson.id,
        userId: session.id,
...
async function moderationHTML(lessonId) {
  const comments = await freedomCommentsStore.getByLessonId(lessonId);
  const rows = comments
    .map(
      (c) => `
      <tr data-id="${c.id}">
...
function wireModeration(container, rerender) {
  container.querySelectorAll('[data-comment-action]').forEach((btn) => {
    btn.addEventListener('click', async () => {
      ...
      if (action === 'approve') await freedomCommentsStore.approve(id);
      else if (action === 'hide') await freedomCommentsStore.hide(id);
      else if (action === 'delete') {
